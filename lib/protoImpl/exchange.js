...
async function getCandles(call, cb) {
    const { handle, exchange, error } = validateHandle(call);
    if (error) {
        return cb(error);
    }
    ...
    const err = isUnsupported(exchange, 'fetchOHLCV');
    if (err) {
        return cb(err);
    }
    ...
    // Assigning granularity here to prevent back-and-forth mapping
    const granularity = (call.request.timeframe || 'tf30m').slice(2); // 30min default
    let since = +call.request.since;
    if (isNaN(since)) {
        since = undefined;
    }
    ...
    let candles = [];
    try {
        candles = await exchange.fetchOHLCV(symbol, granularity, since, limit, params);
    } catch (err) {
        return cb({
            code: grpc.status.NOT_FOUND,
            message: `Error loading orderbook for ${symbol} on ${exchange.name}(${handle}). ${err.message}`
    ...
    return cb(null, mapCandles(candles));
}

async function getTradeHistory(call) {
    let { handle, exchange, error } = validateHandle(call);
    if (error) {
        call.emit('error', error);
        return call.end();
    }
    error = isUnsupported(exchange, 'fetchTrades');
    if (error) {
        call.emit('error', error);
        return call.end();
    }
    ...
    const since = call.request.since;
    const limit = call.request.limit || 1000;
    let params;
    try {
        params = JSON.parse(call.request.params);
    ...
    try {
        ...
        trades.forEach(trade => {
            call.write(mapTrade(trade));
        });
    } catch (err) {
        call.emit('error', {
            code: grpc.status.NOT_FOUND,
            message: `Error loading trade history on ${exchange.name}(${handle}). ${err.message}`
        });
    }
    call.end();
}
...
function mapTrade(trade) {
    const result = Object.assign({}, trade);
    result.info = JSON.stringify(trade.info);
    return result;
}
